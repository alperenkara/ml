...
import numpy as np
"""
from iris_mlp_vector import matrix_mul_bias
from iris_mlp_vector import vec_mat_bias
from iris_mlp_vector import mat_vec
from iris_mlp_vector import sigmoid
"""
random.seed(123)
...
# choose whole rows of the 4th column
# compare and assign with .index() , Iris-setosa==0 Iris-versicolor==1 so on
for row in dataset:
    row[4] = ["Iris-setosa", "Iris-versicolor", "Iris-virginica"].index(row[4])
    row[:4] = [float(row[j]) for j in range(len(row))]

# Split x and y (feature and target)
random.shuffle(dataset)  # randomize list
...
def sigmoid(A):
    ...
    def relu(A): return A * (A > 0).astype(float)
    out = relu(A)
    cache = 0
    return out, cache
...
alfa = 0.005
epoch = 400
neuron = [4, 16, 3]  # number of neuron each layer
